...
def create_app(args):
    ...
    def pubmed_api(pmids):
        ...
        for r in result_dicts:
            ...
            del_keys_from_dict(
                r, ["sourcedb", "sourceid", "project", "elapse_time", "error_code", "error_message"])
    ...
    def plain_api():
        ...
        del_keys_from_dict(result_dict, [
                           "sourcedb", "sourceid", "project", "elapse_time", "error_code", "error_message"])
        ...
        if req_type == "text":
            ...
            if not args.front_dev:
                ...
                dummy_path = os.path.join(
                    app.root_path, "temp/dummy1_20211129.json")
                with open(dummy_path, 'r') as rf:
                    result_dict = json.load(rf)
            _code, parse_res, keys_in_dict = r_parser.parse_result(
                result_dict, draw_keys, result_id="text")
            ...
            if not args.front_dev:
                result_dicts = [model.annotate_pmid(
                    pmid=pmid) for pmid in _pmids]
            else:
                dummy_path = os.path.join(
                    app.root_path, "temp/dummy2_20111129.json")
                with open(dummy_path, 'r') as rf:
                    result_dicts = json.load(rf)

            pmid2result_dicts = {
                f"{result_dict['pmid']}_{i}": result_dict for i, result_dict in enumerate(result_dicts)}

            for _pmid, result_dict in pmid2result_dicts.items():
                _code, parse_res, keys_in_dict = r_parser.parse_result(
                    result_dict, draw_keys, result_id=_pmid)
                ...
                legend_items = {
                    k: r_parser.entity_type_dict[k] for k in keys_in_dict}
