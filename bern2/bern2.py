...
class BERN2():
    def __init__(self,
                 ...
                 disease_norm_port,
                 cache_port,
                 gene_norm_host='127.0.0.1',
                 disease_norm_host='127.0.0.1',
        ...
        self.normalizer = Normalizer(
            gene_host=gene_norm_host,
            disease_host=disease_norm_host,
        ...
        try:
            client = MongoClient(cache_host, cache_port,
    ...
    def annotate_text(self, text, pmid=None):
        try:
            text = text.strip()
            # for the name of temporary files
    ...
    def tag_entities(self, text, base_name):
        ...
        input_gnormplus = os.path.join(
            self.gnormplus_home, 'input', pubtator_file)
        output_gnormplus = os.path.join(
            self.gnormplus_home, 'output', pubtator_file)
        ...
        for ner_type in ['tmvar', 'gnormplus', 'mtner']:
            arguments_for_coroutines.append(
                [ner_type, pubtator_file, output_mtner, base_name, loop])
        async_result = loop.run_until_complete(
            self.async_ner(arguments_for_coroutines))
        loop.close()
    ...
    def resolve_overlap(self, tagged_docs, tmvar_docs):
        ...
        for span in span2mentions.keys():
            ...
            span2mentions[span] = sorted(span2mentions[span], key=lambda x: (
                x['check_CUI'], x['prob']), reverse=True)
    ...
    def generate_base_name(self, text):
        ...
        base_name = hashlib.sha224(
            (text+str(time.time())).encode('utf-8')).hexdigest()
        return base_name
